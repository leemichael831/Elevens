...
public class CardGameGUI extends JFrame implements ActionListener {
	...
	private void initDisplay()	{
		...
		autoButton.setBounds(BUTTON_LEFT, 70 + BUTTON_HEIGHT_INC,
										100, 30);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource().equals(replaceButton)) {
		...
		else if (e.getSource().equals(autoButton)) {//the automatic robot simulation
			...
			if(board.isEmpty()){//if no more cards on board then display win
			...
			for(int i = 0; i<10; i++){
				for(int n=0; n<10; n++){
					if(board.cardAt(n)==null || board.cardAt(i)==null) //if there are blank cards then skip					
					continue;
					if (board.cardAt(i).pointValue() +board.cardAt(n).pointValue()==13){
					board.deal(i);//remove and replace cards
					board.deal(n);
					count++;
					break;
					}
					else if(board.cardAt(i).rank().equals("king")){
						board.deal(i);
						count++;
						break;
					}
				}
				if(count==1)//if found the card stop the loop 
				break;
			}
			repaint();
		}
